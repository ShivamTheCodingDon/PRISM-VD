...
def evaluate_real_world():
    ...
    model_path = "/media/user1/One Touch1/00 Data/PRISM-VD/BigVulW/weights_bigvul_ucg_rgat_rwr_wbce_pw3"
    ...
    model.load_state_dict(torch.load(os.path.join(model_path, "model_best_f1.pt"), map_location=device))
    model.to(device)
    model.eval()
    
    real_world_dir = "/home/user1/AIVul(Don't Delete It)/PRISM-VD/PRISM-VD-Enhanced/real_world_testing/output"
    ...
    for dataset_path in datasets:
        ...
        # FIRST load the cve_db_auto mapping
        ...
        with torch.no_grad():
            for batch in tqdm(dataloader):
                ...
                for prob, true_label in zip(probs, true_labels):
        ...
        for cve, stat in sorted_cves:
            ...
            # Only print CVEs that have more than 1 sample to avoid spam, or all if small
            if stat["total"] > 1:
    ...
    output_json = os.path.join(os.path.dirname(__file__), "real_world_metrics_summary.json")
    with open(output_json, "w") as f:
        json.dump(all_metrics, f, indent=4)
    print(f"\nSaved detailed metrics to {output_json}")
